...
void tickettype(char a[13][6], char &x, int &row, char &seat)     // function to select ticket class
{
	...
	cin >> x;
	cout << endl;
	switch (x)       // switch statment to to perform task according to user choice
	{
	case 'F':
	case'f':
		reserve(a, 1, 2, row, seat);
		break;
	case 'B':
	case'b':
		reserve(a, 3, 7, row, seat);
		break;
	case 'E':
	case 'e':
		reserve(a, 8, 13, row, seat);
		break;
	default:
